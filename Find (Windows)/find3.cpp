...
void setPatchData(xmlDocPtr doc, int numPatches, CvPoint patchPos[], int upsideDown,
	...
	if (result) {
		...
		for (int i=0; i < numPatches; i++) {
			_itoa_s(patchPos[i].x, temp, 10);
			xmlNodeSetContent(nodeset->nodeTab[i], (xmlChar*) temp);	
		}
		xmlXPathFreeObject(result);
	}
	...
	if (result) {
		nodeset = result->nodesetval;
		_itoa_s(numPatches, temp, 10);
		xmlNodeSetContent(nodeset->nodeTab[0], (xmlChar*) temp);
		xmlXPathFreeObject(result);
	}
	...
	if (result) {
		...
		for (int i=0; i < size; i++) {
			_itoa_s(scale*dst_corners[i].x, temp, 10);
			xmlNodeSetContent(nodeset->nodeTab[i], (xmlChar*) temp);	
		}
		xmlXPathFreeObject(result);
	}
	...
	if (result) {
		nodeset = result->nodesetval;
		for (int i=0; i < size; i++) {
			_itoa_s(scale*dst_corners[i].y, temp, 10);
			xmlNodeSetContent(nodeset->nodeTab[i], (xmlChar*) temp);	
		}
		xmlXPathFreeObject(result);
	}
...
//	char* object_filename = argv[1]; 
	...
	char* object_filename = argc == 7 ? argv[1] : "C:/Programmering/Matchning/Linjal.png";
	char* scene_filename = argc == 7 ? argv[2] : "C:/Programmering/Matchning/sceneRGB.tif";
	char* data_filename = argc == 7 ? argv[3] : "C:/Programmering/Matchning/targetData.xml";
	char* saveTemplate_filename = argc == 7 ? argv[4] : "C:/Programmering/Matchning/imageDataTemplate.xml";
	char* save_filename = argc == 7 ? argv[5] : "C:/Programmering/Matchning/imageData.xml";
	char* output = argc == 7 ? argv[6] : "C:/Programmering/Matchning/output.png";
	...
	xmlDocPtr doc = getDoc(data_filename);
	...
	while (object->width > treshold || object->height > treshold) {
		object=doPyrDown(object, IPL_GAUSSIAN_5x5);
		image=doPyrDown(image, IPL_GAUSSIAN_5x5);
	...
	doc = getDoc(saveTemplate_filename);
